...
describe Mail::ReceivedElement do
  ...
  it "should parse elements without date" do
    ...
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq received_text
  end

  it "should parse comment element" do
    received_text = '(qmail 77527 invoked by uid 89); 10 Jun 2004 12:17:55 -0000'
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq '(qmail 77527 invoked by uid 89)' 
    rec.date_time.should eq ::DateTime.parse( '10 Jun 2004 12:17:55 -0000' )
  end
  ...
  it "should parse element with comma" do
    ...
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq 'from 12.110.141.194 (helo=miner) by mrelay.perfora.net with ESMTP (Nemesis), id 1BYPIH-1MVx-0MKyxe-0004FJ' 
    rec.date_time.should eq ::DateTime.parse( 'Thu, 10 Jun 2004 09:08:33 -0400' )
  end
  ...
  it "should parse element with nospace" do
    ...
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq 'from 199.222.69.154 ([199.222.69.154]) by maint3.acm.org with Microsoft SMTPSVC(5.5.1877.197.19)' 
    rec.date_time.should eq ::DateTime.parse( 'Sun, 13 Jun 2004 20:52:40 -0400' )
  end
  ...
  it "should parse element with malformed timezone" do
    ...
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq 'from cemail.ceweb.com (unverified [127.0.0.1]) by lists.catholicexchange.com (SurgeMail 1.3i) with ESMTP id 6639316 for <peter@xxxxxx>' 
    rec.date_time.should eq ::DateTime.parse( 'Wed, 07 Jul 2004 23:08:05 -500' )
  end
  ...
  it "should parse commas in atoms" do
    ...
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq 'by us-bdb-1201.vdc.amazon.com id AAA-default-12226,16' 
    rec.date_time.should eq ::DateTime.parse( '12 Jul 2004 07:44:56 -0700' )
  end
  ...
  it "should parse semi-colon separators" do
    ...
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq 'from 68.9.227.247 (SquirrelMail authenticated user mmueller) by webmail.cs.uri.edu with HTTP' 
    rec.date_time.should eq ::DateTime.parse( 'Mon, 12 Jul 2004 21:58:15 -0400 (EDT)' )
  end
  ...
  it "should parse name_val_list after date" do
    ...
    rec = Mail::ReceivedElement.new( received_text )
    rec.info.should eq 'from localhost (localhost [127.0.0.1]) (uid 0) by stark with local id 00018A65.41F91933.00003206' 
    rec.date_time.should eq ::DateTime.parse( 'Thu, 27 Jan 2005 11:39:15 -0500' )
  end
