...
describe "mail encoding" do
  ...
  it "should handle invalid characters" do
    ...
    if RUBY_VERSION > '1.9'
      ...
      expect(m.subject).to eq "Hello \303 World"
    end
  end
  ...
  it "should decode unknown-to-ruby charsets" do
      ...
      expect { expect(message.decoded.encoding).to eq Encoding::UTF_8 }.not_to raise_error
  end
  ...
  it "should encode utf replacement char as ascii-8bit" do
    ...
    expect { field = field.send( :fold ) }.not_to raise_error
    expect(field).to eq [ "=?UTF-8?Q?<360=EF=BF=BD=EF=BF=BD=EF=BF=BD=EF=BF=BD20131217140814042.jpg@41627.4227978472.75?=" ]
  end
