...
describe Mail::KeywordsField do
  ...
  describe "giving a list of keywords" do
    it "should return a list of keywords" do
      k = Mail::KeywordsField.new('these are keywords, so there')
      expect(k.to_s).to eq 'these are keywords, so there'
    end
    ...
    it "should handle comments" do
      ...
      expect(k.to_s).to eq '"these, are keywords", so there (This is an irrelevant comment)'
    end
    ...
    it "should handle comments in quotes" do
      ...
      expect(k.to_s).to eq '"these, are keywords (another comment to be ignored)", so there (This is an irrelevant comment)'
    end
  ...
  describe "encoding and decoding" do
    it "should encode" do
      k = Mail::KeywordsField.new('these are keywords, so there')
      expect(k.encoded).to eq "Keywords: these are keywords, so there\r\n"
    end
